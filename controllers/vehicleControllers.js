...
/* ══════════════════════════════════════════════════
   PUT /vehicles/:id  —  resident edits a vehicle
   Mirrors addVehicle's validation + slot rules so Web
   and App stay consistent. Supports editing name, number,
   type, flat, and parking slot (link/unlink/change).
   EXTRA slots are released back to the pool on unlink.
   DEFAULT slots stay ASSIGNED to the flat.
══════════════════════════════════════════════════ */
const updateVehicle = async (req, res) => {
  try {
    ...
    /* ── Find the vehicle (society-scoped) ── */
    ...
    /* ─────────────────────────────
       Duplicate Vehicle Check (exclude self)
    ───────────────────────────── */
    const duplicate = await Vehicle.findOne({
      where: {
        vehicle_number: vehicle_number.toUpperCase(),
    ...
    /* ─────────────────────────────
       Validate the NEW selected slot (if any)
       Same three cases as addVehicle.
    ───────────────────────────── */
    let linkedSlotRecord = null;
    ...
    if (parking_slot_id) {
      ...
      if (!linkedSlotRecord) {
        return res.status(400).json({ message: "Selected parking slot not found." });
      }
      ...
      if (linkedSlotRecord.vehicle_type !== vehicle_type.toUpperCase()) {
        return res.status(400).json({ message: "Vehicle type does not match parking slot type." });
      }
      ...
      if (slotInUse) {
        return res.status(400).json({ message: "Selected parking slot is already occupied." });
      }
    }

    /* ─────────────────────────────
       Release the OLD slot (if the slot is changing)
       — EXTRA slot → reset back to the global pool
       — DEFAULT slot → stays ASSIGNED to the flat
    ───────────────────────────── */
    const oldSlotId = vehicle.parking_slot_id;
    const oldNumber = vehicle.vehicle_number;

    if (oldSlotId && oldSlotId !== (parking_slot_id || null)) {
      const oldSlot = await ParkingSlot.findOne({
        where: { id: oldSlotId, society_id: req.user.society_id },
      });

      if (oldSlot && oldSlot.parking_type === "EXTRA") {
        ...
        await ParkingRequest.update(
          { status: "COMPLETED" },
          {
            where: {
              society_id:    req.user.society_id,
              assigned_spot: oldSlot.slot_number,
              parking_type:  "RESIDENT",
              status:        "APPROVED",
            },
          }
    ...
    /* ─────────────────────────────
       Resolve any pending RESIDENT slot requests
       tied to this vehicle (old slot/number no longer apply)
    ───────────────────────────── */
    const { ParkingRequest, Notification, User } = require("../models");
    await ParkingRequest.update(
      { status: "REJECTED" },
    ...
    /* ─────────────────────────────
       Free pre-assigned slot check (same as addVehicle)
    ───────────────────────────── */
    let freePreAssignedSlot = null;
    ...
    /* ─────────────────────────────
       No slot + no free pre-assigned slot → ensure a
       PENDING resident request exists (mirrors addVehicle)
    ───────────────────────────── */
    let residentRequest = null;

    if (!parking_slot_id && !freePreAssignedSlot) {
      let existingRequest = await ParkingRequest.findOne({
        where: {
          society_id:     req.user.society_id,
          vehicle_id:     vehicle.id,
          parking_type:   "RESIDENT",
      ...
      if (!existingRequest) {
        existingRequest = await ParkingRequest.findOne({
          where: {
            society_id:     req.user.society_id,
            vehicle_number: vehicle_number.toUpperCase(),
            parking_type:   "RESIDENT",
    ...
    /* ─────────────────────────────
       Final response (same shape as addVehicle)
    ───────────────────────────── */
    let responseParkingType = "EXTRA";
...
module.exports = { addVehicle, updateVehicle, getMyVehicles, deleteVehicle };